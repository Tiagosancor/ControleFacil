...
export default function NewBankAccountPage() {
  ...
  const submit = async (e) => {
    ...
    setSubmitting(true)
    try {
      await bankAccountService.create({ name, initialBalance: Number(initialBalance), bankIspb: bank?.ispb || null })
    ...
    <AppLayout>
      ...
      <Card className="max-w-lg">
        <form onSubmit={submit}>
          <FormInput label="Nome" value={name} onChange={setName} error={errors.name} placeholder="Ex: Conta salário, Caixinha" />
